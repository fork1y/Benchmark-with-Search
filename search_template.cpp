...
int binarySearch(vector<int> & v, int start, int end, int elem){
    ...
    if (v[mid]>elem){
        end = mid - 1; // smaller elem
    }
    // 2) update start (search right half)
    else if (v[mid]<elem){   //bigger elem
        start = mid + 1;
    }
...
int main(){
    ...
    // create a for loop to iterate through the file sizes
     // get the name/size of the file and assign it to string called filename
        ...
        vecGen (filename, v);
        cout << "Loaded " << v.size() << " numbers from " << filename << endl;
          //  cout<<filename<<endl;
        times.clear();
...
// double elapsed_time_in_sec = us / 1'000'000.0;
...
double elapsed_time_in_sec = ns / 1'000'000'000.0;

cout << index_if_found << ": " << ns << " ns" << endl;   // optional debug print
times.push_back(elapsed_time_in_sec);
            }
    ...
    avg.clear();                    // call avg.clear() to reset avg, so we can use it for binarySearch
